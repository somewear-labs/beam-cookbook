#!/usr/bin/env python3
"""Download USGS satellite imagery for Lacey, WA and package as MBTiles."""
import math
import sqlite3
...
OUTPUT = os.path.join(os.path.dirname(__file__), "lacey_wa_usgs_satellite.mbtiles")
WORKERS = 12
...
def main():
    ...
    total = len(tasks)
    print(f"Downloading {total} tiles (zoom {MIN_ZOOM}-{MAX_ZOOM}) for Lacey, WA...")
    ...
    c.executemany("INSERT INTO metadata VALUES (?, ?)", [
        ("name",        "USGS Satellite - Lacey, WA"),
        ("type",        "overlay"),
        ("version",     "1.1"),
        ("description", "USGS National Map Imagery Only - Lacey, Washington"),
        ("format",      "jpeg"),
    ...
    with ThreadPoolExecutor(max_workers=WORKERS) as pool:
        ...
        for fut in as_completed(futures):
            ...
            with lock:
                ...
                print(f"  {done}/{total} ({errors} errors)", end="\r", flush=True)

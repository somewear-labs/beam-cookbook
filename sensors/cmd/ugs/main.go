...
//
// Wire format: SWL header (type=1) + protobuf UGSData
package main
...
// toF32 converts a []float64 slice to []float32.
...
func generateReading() *pb.UGSData {
	now := time.Now()
	...
	return &pb.UGSData{
		...
		Latitude:        nodeLat,
		Longitude:       nodeLon,
		AltitudeM:       float32(nodeAlt),
		SeismicE:        toF32(seisE),
		SeismicN:        toF32(seisN),
		SeismicZ:        toF32(seisZ),
		SampleRateHz:    100,
		PeakAmplitude:   float32(peak),
		ThreatType:      threat,
		ConfidencePct:   int32(confidence),
		BearingDeg:      float32(bearing),
		EstimatedRangeM: float32(rangeM),
		EventId:         eventID,
	}
}
...
func run(beamURL string, workspaceID int, interval time.Duration, verbose bool) {
	...
	for range ticker.C {
		...
		if verbose {
			fmt.Printf("[ugs] payload: node=%s threat=%s peak=%.2e bearing=%.1f range=%.0f conf=%d\n",
				d.NodeId, d.ThreatType, d.PeakAmplitude, d.BearingDeg, d.EstimatedRangeM, d.ConfidencePct)
		}
		...
		if d.EventId != "" {
			contactNote = fmt.Sprintf(" %s bearing=%.0f° range=%.0fm conf=%d%%",
				d.ThreatType, d.BearingDeg, d.EstimatedRangeM, d.ConfidencePct)
		}
